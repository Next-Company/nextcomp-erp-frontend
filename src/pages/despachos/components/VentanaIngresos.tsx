        ...
        <tfoot>
          ...
          <tr className="h-[45px] font-bold">
            <td>SALDO:</td>
            {
              tallasbase.map(talla=>
                <td>{
                  (data.fracciones.filter(row=>row.talla == talla)[0]?.cantidad ?? 0) - 
                  (data.despachos.length > 0 
                    ? data.despachos.reduce((c1,v1)=>c1 + ( v1.fracciones.length > 0 ? Object.values(v1.fracciones.filter(row=>row.talla == talla)[0] ?? []).filter(item=>typeof item === 'number').reduce((c,v)=>c+v,0) : 0) ,0) : 0)}</td>    
              )
            }
