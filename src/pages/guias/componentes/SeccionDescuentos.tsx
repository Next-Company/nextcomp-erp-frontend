...
export default function SeccionDescuentos({idguia,penalidadestipo,penalidades,setpenalidades}){
  ...
  const nuevoregistro = ()=>{
    ...
    setpenalidades([...penalidades,{idguia:idguia,idx:'',penalidad:'',observacion:'',importe:0}])
  }
  useEffect(()=>{
  ...
  const onclick = (e)=>{
    ...
    switch(action){
      case 'delete':
        setpenalidades(penalidades.filter((row,key)=>key !== parseInt(position) ))
        break;
      default :
    }
  }
  const editvalue = (e)=>{
    if(e.target.dataset.name == 'penalidad'){
      setpenalidades(penalidades.map((row,key)=>key == e.target.dataset.position ? {...row,idx:e.target.value,penalidad:e.target.options[e.target.selectedIndex].text} : row))
    }else{
      setpenalidades(penalidades.map((row,key)=>key == e.target.dataset.position ? {...row,[e.target.dataset.name]:e.target.dataset.name == 'observacion' ? e.target.value : parseFloat(e.target.value)} : row))
    }
  } 
  const agregar = ()=>{
      ...
      <div>
        <div className="scrollbar-special overflow-y-scroll mt-2"> 
          <Table
            ...
            bodycontent={
              penalidades.length > 0 && penalidades.map((row,key)=>(
