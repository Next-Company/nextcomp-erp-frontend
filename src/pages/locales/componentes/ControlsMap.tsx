...
export default function ControlsMap({talleres,settalleres}) {
  ...
  const listartalleres = ()=>{
    console.log("Lllamando a la busqueda de tiendas")
    setLoading(true)
    setSearch('talleres')
    Consulta({url:'locales/getlocalesseguimiento'})
    .then(resp=>{
      console.log("La lista de talleres es :",resp)
      settalleres(resp)
      setShowresultbox(true)
    })
    .catch(err=>{
      toast.error('Se produjo un error en la consulta. Porfavor verifique.',{theme:'colored'})
    })
    .finally(()=>{
      setLoading(false)
    })
  ...
  return(
    <>      
      ...
      <MapControl position={ControlPosition.TOP_LEFT}>
        <div className="p-4">
          <div className='p-3 cursor-pointer hover:bg-gray-200 h-[30px] rounded-full bg-white border shadow-md flex flex-row justify-center items-center text-[14px] font-bold' style={{boxShadow:'0 1px 2px rgba(60,64,67,0.3),0 1px 3px 1px rgba(60,64,67,0.15)'}} onClick={listartiendas}>
      ...
      <MapControl position={ControlPosition.TOP_LEFT}>
        <div className="p-4">
          <div className='p-3 cursor-pointer hover:bg-gray-200 h-[30px] rounded-full bg-white border shadow-md flex flex-row justify-center items-center text-[14px] font-bold' style={{boxShadow:'0 1px 2px rgba(60,64,67,0.3),0 1px 3px 1px rgba(60,64,67,0.15)'}} onClick={listartalleres}>
