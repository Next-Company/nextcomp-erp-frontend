...
export default function TallasCombos({tallasbase,actions}){
  ...
  const addcombos = (e)=>{
    ...
    if(!parseInt(selected)){
      ...
      setInfo(info.filter(talla=>talla !== valor))
    }
  }
  const updatecombos = ()=>{
    actions(info)
            ...
            <tbody>
              {
                tallasbase.tallas.map((row,key)=>(
                  ...
                  <tr key={key} className={`focus-visible:[&_input]:outline-[0px] group focus-visible:[&_input]:bg-gray-200 focus-visible:[&_input]:border-black focus-visible:[&_input]:bg-transparent [&_input]:text-center [&_input]:p-[2px] [&_input]:w-full [&_input]:bg-transparent ${info.includes(row.desc) ? 'selected' : ''}`}>
                    ...
                    <td className="text-center">{row.desc.toUpperCase()}</td>
                    <td className="w-[150px]">
                      <ul className="flex flex-row justify-end">
                        ...
                        <li>
                          <div className="rounded-full w-9 h-9 hover:bg-gray-300 transition-colors flex justify-center items-center" data-position={key} data-action="edit" data-selected={info.includes(row.desc) ? 1 : 0} data-talla={row.desc} onClick={addcombos}>
