...
export function Input({ name, defaults = null, title = "", type, dataset = [], style = {}, action = ()=>{}, mode = null, verify = "false", placeholder = null }) {
    ...
    <>
      <div className={`${type == 'hidden' && 'hidden'} flex-1`}>
        <div onClick={onclick} className={`rounded-md bg-gray-100 flex flex-col justify-start items-start pl-[16px] pr-[16px] pt-[5px] pb-[8px] hover:bg-gray-200 relative box-content group overflow-hidden `} style={style}>
          ...
          <input onClick={(e) => e.stopPropagation()} ref={ref} type={type} onChange={onchange} name={name} defaultValue={initial} onFocus={onfocus} onBlur={onblur}  autoComplete="off" verify={verify} className={`inp bg-[inherit] w-full border-none focus:border-none focus-within:border-none focus-visible:border-none focus:outline-none ${mode ? 'pointer-events-none' : ''}`} min={0.00} step={.01}/>
