...
export function InputSelectT({ title, indices, name, defaults = -1, options }) {
  const ref_menu = useRef(null)
  const [select, setSelect] = useState(defaults == -1 ? DEFAULT_VALUE : defaults)
  const [valores, setValores] = useState(options)
  const [keys, setKeys] = useState(indices)
  ...
  return (
    <>
      <div onClick={onclick} className="rounded-md bg-gray-100 flex flex-col justify-start items-start pl-[16px] pr-[16px] pt-[5px] pb-[8px] hover:bg-gray-200 relative box-content group flex-1">
        ...
        <input onClick={e => e.stopPropagation()} name={name} readOnly value={valores[select]} type="text" onFocus={onfocus} onBlur={onblur} className="inp cursor-default bg-[inherit] w-full border-none focus:border-none focus-within:border-none focus-visible:border-none focus:outline-none" />
