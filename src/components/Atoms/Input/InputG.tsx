export function InputG({ children }) {
  ...
  const alter_label = (e) => {
    ...
    e.target.parentElement.classList.add('pedo')
  }
  const alter_label2 = (e) => {
    if (e.target.value == '') e.target.parentElement.querySelector('label').classList.add('mover')
    e.target.parentElement.classList.remove('pedo')
  }
  return (
    <>
      <div onClick={onclick} className="rounded-md bg-gray-100 flex flex-col justify-start items-start pl-[16px] pr-[16px] pt-[5px] pb-[8px] has-[input:focus]:border-b-blue-600 has-[input:focus]:border-b-[2px] hover:bg-gray-200 relative box-content group">
        ...
        <span className="w-[20px] h-[20px] block bg-red-700"></span>
      ...
      <div className="w-[300px] overflow-hidden border-[0px] rounded-[4px] bg-gray-200 h-[50px] border-red-400 relative group is-published">
        ...
        <span className="w-[20px] h-[20px] block after:absolute after:bottom-0 after:left-0 after:transition-all after:opacity-1 after:w-full after:border-b-[2px] after:border-b-violet-600 group-hover-[.is-published]:bg-red-700 group-hover-[.is-published]:after:border-b-green-600"></span>
      </div>
    </>
  )
}