...
export function InputMultiSelect({ title, name, data, df = null, formref=null, placeholder=null, params={}, position = -1}) {
  ...
  const onblur = (e) => {
    ...
    if(formref){
      const event = new CustomEvent('salamandra', {
        detail:{
          valor:select.map(indice=>data.find((valor,key)=>key == indice)),
    ...
    console.log("Fuera de blur del multiselect")
  }
  ...
  return (
    <>
      <div className="flex-1">
        <div onClick={onclick} onBlur={onblur} className="rounded-md bg-gray-100 flex flex-col justify-start items-start pl-[16px] pr-[16px] pt-[5px] pb-[8px] hover:bg-gray-200 relative box-content group flex-1" tabIndex={-1} data-position={position}>
