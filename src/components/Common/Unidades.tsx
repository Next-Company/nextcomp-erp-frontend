...
export default function UnidadesMedida(children){
  ...
  useEffect(()=>{
    ...
    const buscarproveedor = async ()=>{
      await Consulta({url: 'productos/unidadeslist/100'})
      .then(resp => {
        setLista(resp)
        setLoading(false)
        // setOpenloader(false)
  ...
  const searchunidad = (input,signal)=>{
    setLoading(true)
    const buscarproveedor = async ()=>{
      await Consulta({
        url: 'productos/getunidad/'+ input.value,
        params: {signal}
      })
      .then(resp => {
        setLista(resp)
    ...
    <>
      <div className="flex flex-col mb-2">
        <div className="w-full mb-2">
          <Search config={{ width: '100%' }} action={searchunidad} />
        </div>
          ...
          <table className="w-[100%] border-collapse border-red-100 [&_th]:font-[600] [&_th]:text-center [&_th]:pt-3 [&_th]:pb-3 [&_tr]:border-b [&_td]:p-[6px] [&_tbody_tr:hover]:bg-gray-300 [&_tbody_tr:nth-child(2n-1):hover]:bg-gray-300 text-[12px] [&_tbody_tr:hover]:outline-red-600 [&_tbody_tr:hover]:outline-1 [&_tbody_tr:hover]:outline-double [&_tbody_tr:hover]:cursor-pointer lg:[&_tr:hover_ul]:visible lg:[&_ul]:invisible [&_tbody_tr:nth-child(2n-1)]:bg-gray-100">
            ...
            <tbody>
              {lista.length > 0 
                ...
                : <tr><td className="h-[45px]" colSpan={4}>Datos no encontrados</td></tr>
              }
            </tbody>
          </table>
          {
