...
export default function Pagos(children){
  ...
  let {actions = ()=>{},idref = null} = children
  let [lista,setLista] = useState([])
  useEffect(()=>{
    const buscarproveedor = async ()=>{
      await Consulta({url: 'abonos/getabonobyservicio/'+idref})
      .then(resp => {
        setLista(resp)
        // setOpenloader(false)
    ...
    <>
      <div className="flex flex-col mb-2">
        <div className="w-full mb-2">
          <Search config={{ width: '100%' }} action={()=>{}} />
        </div>
        <div className="h-[600px] w-[1200px] scrollbar-special rounded-md overflow-y-scroll ">
          <table className="w-[100%] border-collapse border-red-100 [&_th]:font-[600] [&_th]:text-center [&_th]:pt-3 [&_th]:pb-3 [&_tr]:border-b [&_td]:p-[6px] [&_tbody_tr:hover]:bg-gray-100 text-[12px] [&_tbody_tr:hover]:outline-red-600 [&_tbody_tr:hover]:outline-1 [&_tbody_tr:hover]:outline-double [&_tbody_tr:hover]:cursor-pointer lg:[&_tr:hover_ul]:visible lg:[&_ul]:invisible [&_tbody_tr:nth-child(2n-1)]:bg-gray-100">
            <thead className="text-left sticky top-0 bg-white">
              <tr>
                <th className="lg:table-cell">Id</th>  
                <th className="lg:table-cell">Entidad Bancaria</th>
                <th className="lg:table-cell">Cuenta Corriente</th>
                <th className="lg:table-cell">TipoOperación</th>
                <th className="lg:table-cell">NúmeroOperación</th>
            ...
            <tbody>
              {lista.length > 0 && lista.map((row,key)=>(
                <tr key={key}>
                  ...
                  <td>{row.moneda == 'D' ? 'Dolares' : 'Soles'}</td>
                  <td>{row.fec_pago}</td>
                  <td>S/.{row.importe}</td>
